...
pub fn main() !void {
    ...
    const range = try NDArray.initArange(&.{ 1, 10 }, 0.0, 10.0, 1.0);
    defer range.deinit();
    range.print("Arange [0, 10, step=1]", 1);
    
    const linsp = try NDArray.initLinspace(&.{ 1, 5 }, 0.0, 1.0, 5);
    defer linsp.deinit();
    ...
    const mat = try NDArray.initArange(&.{ 4, 5 }, 1.0, 21.0, 1.0);
    defer mat.deinit();
    mat.print("Original [4, 5]", 1);
    
    const rows = try mat.initTake(0, 1, 3);  // Rows 1 and 2
    defer rows.deinit();
    rows.print("Rows [1:3]", 1);
    
    const cols = try mat.initTake(1, 1, 4);  // Columns 1, 2, 3
    defer cols.deinit();
    ...
    const t1 = try NDArray.initOnes(&.{ 2, 3, 4 });
    defer t1.deinit();
    
    const t2 = try NDArray.initFull(&.{ 4, 5 }, 2.0);
    defer t2.deinit();
    
    // Contract on axis 2 of t1 and axis 0 of t2
    const tensor_result = try t1.initTensorDot(t2, &[_]i32{2}, &[_]i32{0});
    ...
    const s1 = try NDArray.initOnes(&.{ 2, 3 });
    defer s1.deinit();
    
    const s2 = try NDArray.initFull(&.{ 2, 3 }, 2.0);
    defer s2.deinit();
    
    const s3 = try NDArray.initFull(&.{ 2, 3 }, 3.0);
    defer s3.deinit();
    
    const stacked = try NDArray.initStack(0, &[_]NDArray{ s1, s2, s3 });
    ...
    const c1 = try NDArray.initOnes(&.{ 2, 3 });
    defer c1.deinit();
    
    const c2 = try NDArray.initFull(&.{ 2, 3 }, 2.0);
    defer c2.deinit();
    
    // Concat along axis 0 (rows)
    const concat_rows = try NDArray.initConcat(0, &[_]NDArray{ c1, c2 });
    defer concat_rows.deinit();
    concat_rows.print("Concat along axis 0 (rows)", 1);
    
    // Concat along axis 1 (columns)
    const concat_cols = try NDArray.initConcat(1, &[_]NDArray{ c1, c2 });
